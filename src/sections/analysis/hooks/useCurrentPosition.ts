...
import { isNodeInMainline } from "@/lib/analysisTree";
...
export const useCurrentPosition = (engine: UciEngine | null) => {
  ...
  const currentAnalysisNodeId = useAtomValue(currentAnalysisNodeIdAtom);
  ...
  useEffect(() => {
    ...
    const isOnMainline =
      currentAnalysisNodeId === analysisTree.rootId ||
      isNodeInMainline(analysisTree, currentAnalysisNodeId);
